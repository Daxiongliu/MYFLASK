...
def index():
    ...
    if current_user.is_authenticated:
        show_followed = bool(request.cookies.get('show_followed', ''))      #从cookies中获取登陆用户的选择
    if show_followed:       #显示所关注用户的文章
...
@login_required
def show_all():
    resp = make_response(redirect(url_for('.index')))       #设置响应目标，在响应中设置index需要的cookie
...
def edit_profile():
    ...
    if form.validate_on_submit():
        ...
        current_user.bout_mea = form.about_me.data
...
def edit(id):   #编辑文章
    ...
    if current_user != post.author and \
        not current_user.can(Permission.ADMINISTER):    #检查文章为自己的或有管理员权限
        abort(403)
    form = PostForm()
    if form.validate_on_submit():
